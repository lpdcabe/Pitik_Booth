...
export default function RoomSettings({
  ...
  const count = participantCount || Number(value.maxParticipants) || 2;
  ...
  return (
    <fieldset className="room-settings" disabled={disabled}>
      <div className="field-grid">
            ...
            <select
              ...
              onChange={(e) =>
                onChange({
                  ...
                  layout: getRemoteTemplates(Number(e.target.value))[0].id,
                })
              }
            >
              {[2, 3, 4].map((n) => (
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => change("photoCount", Number(e.target.value))}
          >
            {[1, 2, 3, 4, 6, 8, 9].map((n) => (
              <option key={n} value={n}>
                {n} {n === 1 ? "photo" : "photos"}
        ...
        <label>
          ...
          <select
            ...
            onChange={(e) => change("countdownSeconds", Number(e.target.value))}
          >
            {[3, 5, 10].map((n) => (
              <option key={n} value={n}>
                {n} seconds
      ...
      <label>
        ...
        <select
          value={
            templates.some((t) => t.id === value.layout)
              ? value.layout
              : templates[0].id
          }
      ...
      <div className="remote-template-grid">
        {templates.map((t) => (
          <button
            type="button"
            key={t.id}
            aria-label={`Choose ${t.name}`}
            aria-pressed={value.layout === t.id}
            className={value.layout === t.id ? "selected" : ""}
            onClick={() => change("layout", t.id)}
          >
      ...
      <label className="inline-check">
        <input
          type="checkbox"
          checked={value.autoContinue}
          onChange={(e) => change("autoContinue", e.target.checked)}
        />{" "}
        Automatically start the next photo when everyone approves
      </label>
    </fieldset>
  );
}
